...
//b：渠底宽度，m：边坡系数（矩形时为0），h:水深，n:糙率，i:底坡。
    private double standardstsection_cal(double b,double m,double n,double i,double h)
    {
        double Q = 0,A,X,R,Y;
        try
        {
    ...
    //巴歇尔槽水深流量计算函数
//h：水深；types：巴歇尔槽型号
    private double parshall_cal(double h,int types)
    {
        double C=0,N1=0,Q;
...
//水闸类型：types，过流总宽度：B，闸门开度：a，闸前水深：H，闸后水深：hs，重力加速度按9.81计算
    private double gate_cal(double B,double a,double H,double hs)
    {
        double Q=0;
        try
        {
