...
public class StAlarminfoServiceImpl extends ServiceImpl<StAlarminfoMapper, StAlarminfo> implements StAlarminfoService {
    ...
    //降雨预警信息查询
    public List<StPpAlarm> selectStPpAlarmByStcd(LvDingParam lvDingParam){
        return stAlarminfoMapper.selectStPpAlarmByStcd(lvDingParam);
    }
    //水库水位预警指标统计
    ...
    //实时预警信息 站点预警和短信预警
    public Map<String,Object> selectAlarmMessageByToday(){
        List<StAlarminfo> sitelist=stAlarminfoMapper.selectSiteAlarmByToday();
        List<StAlarminfo> messagelist=stAlarminfoMapper.selectMessageAlarmByToday();
        Map<String,Object> map=new HashMap<>();
        map.put("sitecount",sitelist.size());
        map.put("sitealarm",sitelist);
        map.put("messagecount",messagelist.size());
        map.put("messagealarm",messagelist);
        return map;
    }
}
