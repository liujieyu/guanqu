...
import vip.yingmen.mapper.HeDaoShuiQingGisMapper;
...
@Service
public class HeDaoShuiQingGISServiceImpl implements HeDaoShuiQingGISService {
    @Autowired
    private HeDaoShuiQingGisMapper heDaoShuiQingGISMapper;

    //河道水情的gis第一滑框实时数据查询
    @Override
    public Map findByHeDaoShuiQingGIS(Map searchMap) {
//        System.out.println(searchMap.get("STGR"));
        ...
        List<HeDaoShuiQingGIS> heDaoShuiQingGISList = heDaoShuiQingGISMapper.findByHeDaoShuiQingGIS(searchMap);
        List<HeDaoShuiQingGIS> shuiQingGISList = new ArrayList<>();
        for (HeDaoShuiQingGIS s:heDaoShuiQingGISList){
            if (s.getCV() != null && s.getCV() != "") {
                if (searchMap.get("max_yichang") != null && searchMap.get("max_yichang") != "") {
        ...
        for (HeDaoShuiQingGIS s : shuiQingGISList) {
...
//        System.out.println(searchMap.get("STGR"));
        ...
        map.put("fieldAliases", fieldAliases);
        map.put("geometryType", "esriGeometryPoint");
        List<HeDaoShuiQingGIS> heDaoShuiQingGISList = heDaoShuiQingGISMapper.findByHeDaoShuiQingGIS();
        List<features> featuresList = new ArrayList<>();
        for (HeDaoShuiQingGIS s : heDaoShuiQingGISList) {
            features features = new features();
            attributes attributes = new attributes();
