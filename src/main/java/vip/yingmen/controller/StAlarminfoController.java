...
import vip.yingmen.service.SmsBoxsendedService;
import vip.yingmen.service.StAlarminfoService;
import vip.yingmen.service.WrpFieldinfoService;
...
public class StAlarminfoController {
    ...
    public Map<String,Object> getppalarm(LvDingParam lvDingParam){
        ...
        }else if(lvDingParam.getOrderBy().equals("STLC")){
            lvDingParam.setOrderBy("b.STLC");
        }else if(lvDingParam.getOrderBy().equals("EWLNAME")){
            lvDingParam.setOrderBy("a.EWL");
        }else{
            lvDingParam.setOrderBy("a."+lvDingParam.getOrderBy());
        }
        if(lvDingParam.getCanalname()!=null && !lvDingParam.getCanalname().equals("")){
            ArrayList<String> paramlist=new ArrayList<String>(Arrays.asList(lvDingParam.getCanalname().split(",")));
            lvDingParam.setADDlist(paramlist);
        }
    ...
    //查询降雨预警指标
    @RequestMapping(value="/rainalarm",method = RequestMethod.GET)
    @ResponseBody
    public Map<String,Object> rainalarm(LvDingParam lvDingParam){
        List<StPpAlarm> list=stAlarminfoService.selectStPpAlarmByStcd(lvDingParam);
        Map<String,Object> map=new HashMap<>();
        map.put("rows",list);
        return map;
    }
    ...
    @ResponseBody
    public Map<String,Object> gettodayalarm(){
        return stAlarminfoService.selectAlarmMessageByToday();
    }
}
