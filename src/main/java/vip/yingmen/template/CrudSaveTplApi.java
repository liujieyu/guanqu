 ...
 * @param <S> 更新实体
 */
public abstract class CrudSaveTplApi<T,S,U> {
	...
	protected  JpaTplService<T> jpaTplService;
	    ...
	    @PostConstruct
		 ...
		 protected abstract void addBefore(T enetiy,S saveVo);
		 protected abstract void updateBefore(T enetiy,U saveVo);
	...
	@Transactional
    public ResponseEntity<T> update(@Valid @RequestBody U saveVo) throws URISyntaxException {
    	 Long id = ReflectionUtil.invokeGetter(saveVo, "id");
		 if (id == null) {
	            throw new ReqErrorException("Id 必须有值 ");
	        }
		 return  ResponseEntity.ok(jpaTplService.saveAndCopy(saveVo
			 		,var->this.updateBefore(var, saveVo)
			 ));
    }
